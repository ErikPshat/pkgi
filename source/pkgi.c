...
static const char* content_type_str(ContentType content)
{
    switch (content)
    {
		...
		case ContentTuning: return _("Tunings");
...
static void pkgi_do_tail(const char* atxt, uint32_t acolor)
{
    ...
    pkgi_draw_text(4, bottom_y, PKGI_COLOR_TEXT_TAIL, text);
	pkgi_draw_text(4, bottom_y+18, acolor, atxt);
	pkgi_snprintf(text, sizeof(text), _("L2 R2 change category, L1 R1 flip through the page"));
	//pkgi_draw_text(PKGI_MAIN_HMARGIN, bottom_y+18, PKGI_COLOR_TEXT_TAIL, text);
